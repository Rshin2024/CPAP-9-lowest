 ...
 * arithmetic progression with difference 210), T = 8 or 9.
 *
...
static void gen_primes(uint32_t limit) {
    uint8_t *s = calloc(limit + 1, 1);
    small_primes = malloc(sizeof(uint32_t) * (limit / 2 + 10)); n_small_primes = 0;
...
static void sieve_init(void) {
    gen_primes(B);
    sq = malloc(sizeof(uint32_t) * n_small_primes);
    sq_minv = malloc(sizeof(uint32_t) * n_small_primes);
    sq_delta = malloc(sizeof(uint32_t) * n_small_primes);
    nsq = 0;
    ...
    sq_nxt = malloc(sizeof(uint32_t) * (size_t)nsq * T);
}
...
static void search_config(mpz_t x, uint64_t klo, uint64_t khi, int *unc, int nunc) {
    ...
    uint64_t words = L / 64;
    for (uint64_t base = klo; base < khi; base += L) {
        memset(seg, 0, L / 8);
...
static uint64_t *hkey; static uint32_t *hcnt; static uint32_t hused = 0;
static uint32_t seen_key(uint64_t key) {  /* returns previous count, increments */
    if (!hkey) { hkey = calloc(HSIZE, sizeof(uint64_t)); hcnt = calloc(HSIZE, sizeof(uint32_t)); }
    if (key == 0) key = 1;
    if (hused > HSIZE * 3 / 4) return 0;   /* table full: stop deduplicating */
    uint32_t h = (uint32_t)((key * 0x9E3779B97F4A7C15ULL) >> (64 - HBITS));
    while (hkey[h] && hkey[h] != key) h = (h + 1) & (HSIZE - 1);
    if (!hkey[h]) { hkey[h] = key; hcnt[h] = 1; hused++; return 0; }
    return hcnt[h]++;
}
static uint32_t seen_count(mpz_t x) {
    uint64_t key = mpz_get_ui(x) ^ (mpz_fdiv_ui(x, 4294967291u) << 32) ^ ((uint64_t)mpz_fdiv_ui(x, 2147483647u) << 17);
    return seen_key(key);
}
...
typedef struct { uint32_t base; uint8_t nsw; uint8_t sw[3][2]; uint8_t u; } variant_t;
...
static uint8_t cur_sw[3][2];

static void bm_record(int r, int u, int depth, uint32_t base) {
    if (vn >= bm_maxvar) return;
    if (vn == vcap) { vcap = vcap ? vcap * 2 : 1 << 16; vlist = realloc(vlist, vcap * sizeof(variant_t)); vkeys = realloc(vkeys, vcap * sizeof(vkey_t)); }
    variant_t *v = &vlist[vn]; v->base = base; v->nsw = depth; v->u = u;
    for (int i = 0; i < 3; i++) { v->sw[i][0] = i < depth ? cur_sw[i][0] : 0; v->sw[i][1] = i < depth ? cur_sw[i][1] : 0; }
...
static int run_bases_mode(int P, int maxunc, long gidx, long gnum, uint64_t klo, uint64_t khi) {
    ...
    char line[8192]; int nb = 0; int (*bases)[MAXSP + 2] = malloc(sizeof(int) * (MAXSP + 2) * 200000);
    bm_smax = 2;
    while (fgets(line, sizeof line, f)) {
        if (line[0] == '#') {
            char *p;
            if ((p = strstr(line, "SWAPS="))) bm_smax = atoi(p + 6);
            if ((p = strstr(line, "LAYER0="))) bm_layer0 = atol(p + 7);
            if ((p = strstr(line, "LAYERS="))) bm_nlayers = atol(p + 7);
            if ((p = strstr(line, "MAXVAR="))) bm_maxvar = (size_t)atol(p + 7);
            if ((p = strstr(line, "QUOTA="))) bm_quota = (size_t)atol(p + 6);
            if ((p = strstr(line, "REMAP="))) {
                /* REMAP=suffix:wid,... : processes whose git branch ends with suffix get worker id wid (4 procs/worker) */
                char head[512] = ""; FILE *hf = fopen(".git/HEAD", "r");
                ...
                while (tk) {
                    ...
                    if (colon) { *colon = 0; int wid = atoi(colon + 1); size_t sl = strlen(tk);
                        size_t hl = strlen(head); while (hl && (head[hl-1] == '\n' || head[hl-1] == '\r')) head[--hl] = 0;
                        if (sl && hl >= sl && strcmp(head + hl - sl, tk) == 0 && gidx >= 0) { long c = gidx % 4; gidx = wid * 4 + c; printf("REMAP: branch %s -> worker %d, process index %ld\n", head, wid, gidx); }
                    }
                    tk = strtok(NULL, ", \n");
                }
            }
    ...
    /* deduplicate by key (keep lowest enumeration index), then sort by (u, idx) */
    qsort(vkeys, vn, sizeof(vkey_t), cmp_key);
    size_t m = 0;
    for (size_t i = 0; i < vn; i++) {
        if (i > 0 && vkeys[i].key == vkeys[i - 1].key) continue;
        vkeys[m++] = vkeys[i];
    }
    size_t ndup = vn - m;
    for (size_t i = 0; i < m; i++) vkeys[i].key = ((uint64_t)vlist[vkeys[i].idx].u << 32) | vkeys[i].idx;
    qsort(vkeys, m, sizeof(vkey_t), cmp_qual);
    variant_t *sorted = malloc(sizeof(variant_t) * (m ? m : 1));
    ...
    free(vlist); vlist = sorted; vn = m; free(vkeys); vkeys = NULL;
    {
        ...
        for (size_t i = 0; i < vn; i++) { su += vlist[i].u; int d = bm_maxunc - vlist[i].u; if (d > 7) d = 7; hist[d]++; }
    ...
    for (long L = bm_layer0; L < bm_layer0 + bm_nlayers; L++) {
        uint64_t lk = klo + (uint64_t)L * KX, hk = lk + KX;
        ...
        for (size_t i = 0; i < vn; i++) {
            ...
            for (int s2 = 0; s2 < v->nsw; s2++) b2[v->sw[s2][0]] = v->sw[s2][1];
            build_x(r, b2, x, unc, &nunc);
            if (nunc != v->u) { fprintf(stderr, "BUG: variant unc mismatch %d vs %d\n", nunc, v->u); exit(1); }
            snprintf(cfg_desc, sizeof cfg_desc, "bases layer=%ld idx=%zu base#%u swaps=%d unc=%d r=%d k=[%llu,%llu)", L, i, v->base, v->nsw, nunc, r, (unsigned long long)lk, (unsigned long long)hk);
            st_cfg++;
            if (KX <= 2) { printf("DRY %s\n", cfg_desc); continue; }
...
int main(int argc, char **argv) {
    ...
    for (int a = 1; a < argc; a++) {
        if (!strcmp(argv[a], "-P")) P = atoi(argv[++a]);
        else if (!strcmp(argv[a], "-X")) { char *s = strdup(argv[++a]), *tok = strtok(s, ","); while (tok) { excl[nexcl++] = atoi(tok); tok = strtok(NULL, ","); } }
        else if (!strcmp(argv[a], "-T")) T = atoi(argv[++a]);
        else if (!strcmp(argv[a], "-B")) B = (uint32_t)atol(argv[++a]);
        else if (!strcmp(argv[a], "-L")) L = (uint32_t)atol(argv[++a]);
        else if (!strcmp(argv[a], "-k")) klo = strtoull(argv[++a], 0, 10);
        else if (!strcmp(argv[a], "-K")) khi = strtoull(argv[++a], 0, 10);
        ...
        else if (!strcmp(argv[a], "-n")) nconfigs = atol(argv[++a]);
        else if (!strcmp(argv[a], "-i")) saiters = atol(argv[++a]);
        else if (!strcmp(argv[a], "-u")) maxunc = atoi(argv[++a]);
        else if (!strcmp(argv[a], "-o")) hitfile = argv[++a];
        else if (!strcmp(argv[a], "-e")) statevery = atoi(argv[++a]);
        else if (!strcmp(argv[a], "-v")) maxvar = atoi(argv[++a]);
        else if (!strcmp(argv[a], "-w")) baseunc = atoi(argv[++a]);
        else if (!strcmp(argv[a], "-g")) gidx = atol(argv[++a]);
        else if (!strcmp(argv[a], "-G")) gnum = atol(argv[++a]);
    ...
    if (hitfile) hitf = fopen(hitfile, "a");
    st_t0 = now_sec();
    rng_seed(seed);
    mpz_t x; mpz_init(x);
    int unc[NPOS], nunc;
    ...
    if (cfgfile) {
        ...
        while (fgets(line, sizeof line, f)) {
            ...
            while (tok) { int pp, bb; if (sscanf(tok, "%d:%d", &pp, &bb) == 2) { for (int pi = 0; pi < nsp; pi++) if (sp[pi] == pp) b[pi] = bb; } tok = strtok(NULL, " \n"); }
            int ok = 1; for (int pi = 0; pi < nsp; pi++) if (b[pi] < 0) ok = 0;
            if (!ok) { fprintf(stderr, "config missing residues, skipped\n"); continue; }
    ...
    if (gnum > 0) printf("process %ld of %ld: k ranges [(%ld + n*%ld)*%llu + 1, ...) per offset occurrence n\n", gidx, gnum, gidx, gnum, (unsigned long long)KX);
    for (long c = 0; c < nconfigs; c++) {
        ...
        char *xs = mpz_get_str(NULL, 10, x);
        uint64_t rlo = klo, rhi = khi; unsigned rep = 0;
        if (gnum > 0) { rep = seen_count(x); if (rep) st_rep++; rlo = (uint64_t)(gidx + (uint64_t)rep * gnum) * KX + 1; rhi = rlo + KX; }
        snprintf(cfg_desc, sizeof cfg_desc, "seed=%llu cfg#%ld unc=%d r=%d rep=%u k=[%llu,%llu)", (unsigned long long)seed, c, nunc, r, rep, (unsigned long long)rlo, (unsigned long long)rhi);
        printf("CONFIG %ld x=%s unc=%d r=%d tries=%d rep=%u klo=%llu\n", c, xs, nunc, r, tries, rep, (unsigned long long)rlo); fflush(stdout);
        free(xs);
        st_cfg++;
        ...
        for (int pi = 0; pi < nsp && nv < maxvar; pi++) {
            ...
            for (int ai = 0; ai < nallowed[pi] && nv < maxvar; ai++) {
                ...
                if (gnum > 0) { rep = seen_count(x); if (rep) st_rep++; rlo = (uint64_t)(gidx + (uint64_t)rep * gnum) * KX + 1; rhi = rlo + KX; }
                snprintf(cfg_desc, sizeof cfg_desc, "seed=%llu cfg#%ld var%d(%d:%d) unc=%d r=%d rep=%u k=[%llu,%llu)", (unsigned long long)seed, c, nv, sp[pi], nb, nunc, r, rep, (unsigned long long)rlo, (unsigned long long)rhi);
                nv++; st_cfg++; st_var++;
                search_config(x, rlo, rhi, unc, nunc);
                if (st_cfg % statevery == 0) print_stats(stdout);
            }
